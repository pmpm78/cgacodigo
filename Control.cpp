...
void CControl::controlTeclado()
{
	...
	//Enter
	if(GetKeyState(VK_RETURN) & 0x80) //Si está presionada la tecla ENTER
	{
		estadoTecla[6]=1;
	}

	if(!(GetKeyState(VK_RETURN) & 0x80)) //Si no está presionada la tecla ENTER
	{
		estadoTecla[6]=0;
	}

	//Teclas para el proyecto
